...
class AntiplagiarismEngine:
    ...
    def analyze_document(
        ...
        """
        Analyse a document for plagiarism.

        Parameters
        ----------
        file_path       : PDF, TeX, or plain-text file to check
        threshold       : minimum cosine similarity to flag a chunk (0–1)
        top_k           : nearest neighbours to retrieve per chunk from FAISS
        arxiv_id        : if set, fetches LaTeX from arXiv AND excludes that
                          paper from results (self-match exclusion)
        paraphrase_mode : when True, runs the cross-encoder reranker after
                          FAISS retrieval to catch meaning-preserving rewrites
                          that fall below the cosine similarity threshold.
                          Requires use_reranker=True at engine init.
                          Adds ~2–5s to processing time per document.
        """
        if not self.is_ready:
            return {"error": f"Engine not ready: {self.init_error}"}

        # Paraphrase mode is only active when the reranker was successfully loaded
        ...
        if not chunks:
            return {"error": "Document is empty after truncation."}
        ...
        timings["encoding_s"] = round(time.monotonic() - t0, 3)

        # ── 3. Route ───────────────────────────────────────────────────
        routing = self._decide_routing(query_vectors)

        # ── 4. Search ──────────────────────────────────────────────────
        # In paraphrase mode, FAISS casts a wider net at the lower retrieval
        # threshold; the reranker then filters the expanded candidate set.
        retrieval_threshold = (
        ...
        timings["search_s"] = round(time.monotonic() - t0, 3)
        ...
        word_coverage = flagged_words / total_words if total_words else 0.0

        # source_weighted_score = contribution of the single worst source.
        # More intuitive than word_coverage / n_sources:
        #   1 source copying 50% → 50%  (alarming)
        #   10 sources each copying 5% → 5%  (less alarming)
        if ranked_sources:
            worst_source_words = max(s.flagged_word_count for s in ranked_sources)
            source_weighted = worst_source_words / total_words if total_words else 0.0
        else:
            source_weighted = 0.0

        timings["ranking_s"] = round(time.monotonic() - t0, 3)
        timings["total_s"] = round(sum(timings.values()), 3)

        return {
            # ── Scores ────────────────────────────────────────────────
            "word_coverage_percent":  round(word_coverage * 100, 2),
            "source_weighted_score":  round(source_weighted * 100, 2),

            # ── Document stats ────────────────────────────────────────
            "total_chunks":           len(chunks),
            "total_words":            total_words,
            "flagged_chunks_count":   len(flagged_chunk_map),
            "flagged_words_count":    flagged_words,
